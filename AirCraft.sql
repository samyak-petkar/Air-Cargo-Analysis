...
--  6) query to extract the customers who have registered and booked a ticket alter 
select * from customer 
where customer_id in 
(select distinct customer_id from ticket_details)
order by customer_id ;

-- 7) identify the customer’s first name and last name based on their customer ID and brand (Emirates)
select customer_id , first_name, last_name from customer
where customer_id in
(select customer_id from ticket_details where brand = 'EMIRATES')
order by customer_id ;

-- 8)identify the customers who have travelled by Economy Plus class
SELECT * FROM PASSENGERS_ON_FLIGHTS  
GROUP BY SEAT_NUM
